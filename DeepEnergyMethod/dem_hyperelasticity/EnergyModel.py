...
class EnergyModel:
    ...
    def NeoHookean2D(self, u, x):
        ...
        if ('model' not in locals()) and ('model' not in globals()):
            model = torch.load(r'C:\Users\johnn\tubCloud\Uni\Master\MA\Sourcecode\CANN\exports\CANN.pt')
            #model.eval()
            #print('CANN model loaded')

        F = torch.stack([torch.cat([Fxx, Fxy], dim=1), torch.cat([Fyx, Fyy], dim=1)], dim=2)        
        #torch.save(F, 'C:\\Users\\johnn\\tubCloud\\Uni\\Master\\MA\\Sourcecode\\CANN\\datasets\\tensor.pt')
        #print('saved F list to file')
        #strainEnergy = model(F)
        I1 = trC
        C = torch.matmul(F.transpose(-1,-2),F)
        I1 = torch.einsum('ijj -> i', C)
        #I2 = 0.5 * (I1**2 - (C * C).sum(dim=(-1, -2)))
        #I1_ = I1/(detF**(2/3))
        J = torch.det(F)
        d = 0.0005
        strainEnergy = self.mu/2 * (I1 - 2) + (1/d) * (J - 1)**2 #+ self.lam/2 * (J - 2)**2
        ########################################
